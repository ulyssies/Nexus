...
- **Email agent** (read-only Gmail, `gmail.readonly` — never sends/deletes) — `*/15 * * * *` + manual. Batch-classifies importance/category/deadline/job-signal/role with Claude, writes `email_flags`, extracts **real** deadlines into `calendar_events` (promo/noise "offer expires" emails are *not* treated as deadlines and never hit the calendar), flips `jobs.status` on application movement, **and creates a `source='email'` application row when the company isn't already tracked** (e.g. you applied on LinkedIn). Graceful `NO_CREDENTIALS` / `NEEDS_AUTH`. One-time `npm run gmail:auth`.
...
- Keep `CLAUDE.md` in sync with this file (this is the Codex mirror — same content, only the title/intro differ; the **Claude model** references stay "Claude").
