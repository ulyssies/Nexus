...
This file is the source of truth for AI coding tools (Codex, Codex, etc.) in this project. Read it fully before acting. Sub-agents must read it before any scoped work.
...
- **Two clean pieces (React frontend / Express backend)** maps onto the familiar Vercel+Render mental model, just both local. Sustainable, expandable, easy for Codex / Codex to reason about.
...
|---|---|---|---|---|
| **Job agent** | ✅ built · pipeline absorbed, cron + manual run + boot catch-up, rich detail panels, status tracking | `node-cron 0 7 */3 * *` + manual "run now" button + boot catch-up (>72h) | `server/resumes/*.tex`, Adzuna/Muse/Jobicy APIs | `jobs` + `job_seen_keys` tables, sends email report |
...
- **Job agent (done):** the pipeline lives in `server/agents/jobAgent.js` — fetch (Adzuna/Jobicy/The Muse via native `fetch`, no axios) → score in DA/SWE batches against `server/resumes/*.tex` with Codex (`Codex-sonnet-4-6`, résumé prompt-cached) → write via `db/jobsRepo.js` → optional Nodemailer digest. Settings (cities, search terms, thresholds, schedule, résumé paths, `JOB_AGENT_CATCHUP_HOURS`) are in `server/config.js`. Writes share one upsert with the migration so a re-fetch updates the same row and preserves any advanced `jobs.status`. `runJobAgent()` is called by the boot cron and by `POST /api/jobs/run`; the UI polls `GET /api/jobs/run/status`. Note: the original `parseResume`/`extractSkills` step is intentionally not wired in — the scorer consumes the full résumé directly (in the source it only logged a profile).
  - **Rich detail + descriptions:** provider descriptions are no longer truncated at 250 chars — `jobAgent.js` stores cleaned text up to 4000 chars and feeds the full body to the scorer, whose prompt now returns structured `roleSummary`, `responsibilities`, `alignedStrengths`, `positives`, `negatives` (plus the existing `missingSkills`, `reason`, `estimatedSalary`). These land in `match_reasons` and `routes/jobs.js` exposes them; the UI renders an inline detail panel per row. Scoring `max_tokens` raised to 8000 for the richer JSON. Legacy rows without `roleSummary` show a pending-summary message until the next scoring pass.
  - **Status tracking:** manual `Mark as Applied` (`POST /api/jobs/:id/applied`) and full manual status correction (`POST /api/jobs/:id/status`, the existing enum: new/interested/applied/interviewing/offer/rejected/withdrawn/archived). Setting back to `new` clears manual timestamps; application states stamp `status_updated_at`/`status_updated_by='user'` and fill `applied_at`.
  - **Lifetime seen-set:** `job_seen_keys (source, external_id)` records every provider listing ever seen, independent of the live `jobs` table — so `jobs` can be purged while the UI still reports total seen. `getSeenJobKeys()` only treats rows with a substantial (400+ char) description as seen, so thin/legacy snippets are re-fetched and hydrated on later runs.
  - **Retention + boot catch-up:** `purgeStaleJobs({ days: 30 })` deletes only untouched found jobs (`status='new'`, no `applied_at`/`status_updated_at`, age by `posted_at` → `created_at`) and runs after manual runs and on the cron; advanced statuses are never purged. On boot, the server runs the agent in the background (`trigger='boot'`) when the last scan/import is older than `JOB_AGENT_CATCHUP_HOURS` (72h). The job board API also applies the 30-day live rule by default so stale untouched rows are hidden before the next purge.
...
├── master.html              # design source of truth (brief view removed; Phase-1+ build doc lives in AGENTS.md/README)
├── AGENTS.md
...
│   └── views/
│       ├── HomeView.jsx     # command center, 5 zones (from /api/home): alert strip (urgent/actionable only) · today's agenda (goals w/ one-click check-in, a self-contained MiniCalendar month-grid widget reading /api/calendar, job deadlines) · topic-labeled brief digest (renderRich bolds the **labels**) + read-more · scrollable agent feed (filter bar + archivist collapse) · agent health row (dot/last-run/next-run/insight). onNavigate jumps to any tab. MiniCalendar styles are inline (kept to this file)
│       ├── JobsView.jsx     # Job board: Found-by-agent vs Live-applications subtabs, filter bar (track/level/status/city/match-score), company grouping (5+ listings), sort (newest/score/company), 50-row pagination, inline AI detail panels, manual status controls + "run now" & polling
│       ├── JournalView.jsx  # free-form journal: write + save (auto-tagged) + recent entries
│       ├── GraphView.jsx    # second brain split-pane: force graph (left) + rich node panel (right, slides in on click, draggable divider persisted in localStorage). Nodes weighted by connection count, colored by node_type; node names show on HOVER only (no always-on canvas labels) and forces are tuned (strong charge repulsion + loose tag-link strength) so the graph spreads out instead of blobbing; solid hierarchy edges vs dashed tag edges; cluster-focus mode on concept click (cluster full-opacity, rest faded); search-highlight + node-type show/hide toggles. Right panel: type badge, full body (/notes/:id), clickable tags (→ filter)/parent/children/connected, created date, source agent, close X
│       ├── CouncilView.jsx  # Council of 5: ask box, elder cards + stances, consensus meter
│       ├── GoalsView.jsx    # MERGED Goals + Accountability tab: stat cards · goals list (add form, per-goal streak + progress bar, today's check-in done/partial/missed, DELETE button) · accountability agent nudge card (run now). Routed for both 'goals' and the legacy 'accountability' id. (AccountabilityView.jsx removed — folded in here.)
│       ├── CalendarView.jsx # three resizable panels (draggable dividers): full month calendar grid (source-colored chips, click-day expand, prev/next nav) · paginated inbox (filter tabs all/urgent/important/job-alert/newsletter/noise + counts, noise hidden by default, expandable detail rows w/ cross-agent action badges) · agent-rail insight cards (plain-language, from /api/email/insights)
...
└── server/                  # nested Express package — long-running agent host (port 3001)
    ├── package.json         # backend deps + scripts (migrate:jobs, purge:jobs, gmail:auth, seed:brain)
    ├── .env.example         # .env itself is gitignored
    ├── index.js             # Express app + node-cron registration for all 6 agents on boot
    ├── config.js            # agent settings: cities/terms/thresholds/résumés + JOB_AGENT_CATCHUP_HOURS + all cron schedules + WATCHED_PROJECTS + Gmail/news config
    ...
    │   ├── tagAgent.js      # auto-tags a note on save (Codex); graceful no-key fallback
    ...
    │   ├── morningBriefAgent.js    # learn interests → NewsAPI → Codex condense → daily brief; buildDigest() synthesizes the home 3-4 sentence digest (cached on morning_brief.digest, re-called only when stale >6h or forced)
    ...
    │   ├── archivistAgent.js  # git log → Codex {summary,why,impact} → project_changes + tagged graph node; chokidar HEAD watch
    ...
    │   ├── briefRepo.js     # morning_brief + items; learnInterests() + brief_interests CRUD + effectiveInterests() (user steering tags + learned)
    │   ├── emailRepo.js     # email_flags + calendar_events; findJobByCompany + setJobStatus (cross-agent bridge); listFlagsPaged/flagCounts (inbox filter tabs + pagination) + emailInsights (read-only agent-rail derivation)
    │   ├── projectChangesRepo.js  # project_changes upsert + mirrors each change into a graph note
    │   ├── overviewRepo.js  # home dashboard reads: getOverview() (legacy snapshot) + getHome() (5-zone command center: alerts, agenda w/ today's check-in status, agent health w/ last/next run + insight line, cached brief digest, full feed)
    ...
    ├── routes/
    │   ├── jobs.js          # job board API (filters/sort/pagination/grouping, exposes detail fields) + POST /run, GET /run/status, POST /:id/applied, POST /:id/status
    ...
    │   ├── brief.js         # GET / (today's brief + interests), POST /run, POST /digest (cached home digest paragraph; ?force=1 to regen), interests CRUD
    │   ├── email.js         # /status, /stats, /counts, /insights, /flags (paged+filtered; legacy ?importance= still works), POST /run
    │   ├── calendar.js      # GET / (upcoming, user+email events), POST / (user event)
    │   ├── overview.js      # GET /api/overview (legacy snapshot) + homeRouter → GET /api/home (the 5-zone command center)
    ...
    └── scripts/
        ...
        ├── purge-stale-jobs.js  # delete only untouched found jobs (status='new', no applied/manual status) >30d old
        ├── gmail-auth.js    # one-time interactive Gmail authorization (paste-code flow)
        └── seed-second-brain.js  # seed a demo concept hierarchy + tagged child nodes (source_agent='seed'; --reset to rebuild)
```
...
- **DB tables (all created in `server/db/schema.sql`):** `jobs`, `job_seen_keys` (lifetime seen-set, independent of live `jobs`), `email_flags`, `calendar_events`, `goals`, `checkins`, `streaks`, `notes`, `tags`, `note_tags`, `morning_brief`, `morning_brief_items`, `project_changes`, `council_sessions`, `council_responses`, observability/steering: `agent_runs`, `agent_usage`, `brief_interests`, and research: `research_sessions`, `research_messages`, `research_open_questions`. The `notes` table also gained hierarchy columns (`parent_id`, `node_type`, `is_concept`), and `morning_brief` gained `digest` + `digest_at` (the cached home digest paragraph), via additive migrations in `db/index.js` (SQLite can't add columns through `CREATE TABLE IF NOT EXISTS`). All tables are written by their agents.
...
2. **Phase 2 — Job agent ✅ done:** pipeline absorbed into `server/agents/jobAgent.js`, settings in `server/config.js`, 3-day `node-cron` registered on boot (also calls `purgeStaleJobs()`), manual "run now" button + status polling, Job board live from the DB, README written, brief view retired, external repo dependency removed. **Job Board upgrade (latest):** full descriptions (4000-char) + structured AI detail (`roleSummary`/`responsibilities`/`alignedStrengths`/`positives`/`negatives`) in inline panels; Found-by-agent vs Live-applications subtabs; manual status tracking (`/applied`, `/status`); filters/sort/company-grouping/pagination; `job_seen_keys` lifetime seen-set; refined 30-day retention (untouched-only) + boot catch-up (>`JOB_AGENT_CATCHUP_HOURS`).
...
6. **Phase 6 — Project archivist ✅ built:** `projectChangesRepo.js` + `archivistAgent.js` + `routes/projects.js` + `ProjectsView.jsx`. Git-log poll (30 min) + chokidar `.git/logs/HEAD` watch → Codex `{summary, why, impact}` → `project_changes` + auto-tagged `kind='project'` graph node. Sandboxed to `WATCHED_PROJECTS` (defaults to the Nexus repo). Verified live (7 commits → tagged nodes → 21 links; re-scan dedups). ⬜ owner adds their own repos to `WATCHED_PROJECTS`; optional `file_save` change_type later.
7. **Home command center ✅ built → ✅ rebuilt as a 5-zone daily dashboard:** `GET /api/home` (`getHome()` in `db/overviewRepo.js`, mounted via `homeRouter` in `routes/overview.js`) aggregates in one read: **Zone 1** alert strip (only time-sensitive/actionable: calendar+email deadlines ≤7d, goals due today, urgent emails — hidden when empty); **Zone 2** today's agenda (active goals with today's check-in status + one-click check-in, a **compact MiniCalendar month-grid widget** — source-colored event chips, click-a-day popover, prev/next month nav, reads the same `/api/calendar` events as the Calendar view — and job deadlines ≤7d); **Zone 3** the morning-brief **digest** — a cached Codex-written digest of **4–6 topic-labeled, actionable sentences** (`**AI & Engineering:**` / `**Career:**` / `**Learning:**`, one concrete line per relevant story, NOT literary synthesis — see the prompt comment in `morningBriefAgent.js`), via `buildDigest()` (regenerated only when stale >6h or forced), with read-more articles, or a "generate brief" button; **Zone 4** the full scrollable agent feed (filter bar all/jobs/email/accountability/brief/archivist + archivist-collapse toggle); **Zone 5** agent health cards (dot from observability + last-run/next-run via cron-parser + one real insight line per agent). `HomeView.jsx` renders all five; `onNavigate` jumps to any tab. The legacy `GET /api/overview` snapshot is kept. Verified live against the real DB (digest synthesis + caching confirmed). ⬜ retake `docs/screenshots/home.png` + `graph.png` (no headless browser in this env).
8. **Phase 8 — Observability & trust ✅ mostly done:** ✅ instrumented every agent run + Codex call (`agents/claudeClient.js` → `agent_runs`/`agent_usage`; `withRun`/`startRun`/`finishRun`; estimated cost from published per-model rates). ✅ `db/observabilityRepo.js` + `routes/observability.js` (`GET /api/observability`: per-agent last/next run, errors, cost per agent/day; next-run via `cron-parser`). ✅ `SettingsView.jsx` (gear) — cost cards, agent run/cost rows, daily trend, error log, + **brief interest-tag steering** (`brief_interests` table; `effectiveInterests()` merges user picks with learned). ✅ council voices tuned (concise, journal-grounded, Zeno bites). ⬜ remaining: first test suites per layer; prompt-cache cost audit; further triage/nudge tuning.
9. **Phase 9 — Research agent + hierarchical second brain ✅ built:** ✅ **Research agent** end-to-end (`db/researchRepo.js` + `agents/researchAgent.js` + `routes/research.js` + `ResearchView.jsx`): chat sessions (paste text / `fetchUrl` / Q&A), ephemeral conversation, and **save session** → Codex distills it into ONE structured node (topic, summary, key-concept tags, conclusions, **open questions** in `research_open_questions`, sources) saved to the second brain (node_type='research', tagged → joins the graph). Verified live (vector-clocks + caching sessions → clean nodes + tracked open questions). ✅ **Hierarchy** layered additively: `notes.parent_id`/`node_type`/`is_concept` (migration in `db/index.js`); `createConcept`/`setParent`/`listParents`; `getGraph` emits **directed** parent→child links (`directed:true`, drawn with arrows) alongside flat tag edges; `GraphView` renders + color-codes them; Research "file under" picker + concept creation. **Tagging agent's flat shared-tag logic untouched.** Note: `node_type` is the new taxonomy column (legacy `kind` kept — a SQLite CHECK can't be altered in place). ⬜ next: the "Ask Nexus anything" query layer over the accumulated nodes; resurface open questions in the brief.
...
13. **Phase 13 — Hardening:** DB encryption at rest + backup/restore; schema-migrations framework; indexing/pagination/graph virtualization at scale.

   The full, prose version of Phases 8–13 lives in the README roadmap. Order is not locked — the shared-context architecture lets any of these slot in without rewiring the rest, so follow whatever matters most in daily use. **This is the worst version of Nexus it will ever be; everything from here is making it better.**
...
- Job Board detail panels need a fresh scoring pass to populate: rows scored before the structured-fields upgrade have no `roleSummary`/`responsibilities` and show a pending-summary message until re-scored. Providers can only hydrate a thin row when they still return useful description text for that listing. A real retention sweep this work deleted 1,123 untouched stale jobs; counts settled at ~2,016 lifetime seen, ~893 live rows, 0 stale-untouched beyond the window.
...
- **Council persona prompts in `agents/councilAgent.js` are a tuned v2** — the charter now mandates concision (3–5 sentences) and grounding each reply in the owner's journal/goals; each voice is more distinct and Zeno genuinely challenges. Iterate further as used. A council question costs ~11 calls (5+5 Sonnet + 1 Haiku consensus), now visible in the Settings cost panel.
...
- **Morning brief is now live.** A real `NEWS_API_KEY` is set; a run curated 6 real stories (Codex-condensed, topic-mapped to interests learned from the journal). Still degrades gracefully if the key is removed. (Historical gotcha: a `KEY= # comment` line parses as empty in dotenv — keep values free of inline comments.)
...
Detailed session history lives in `.Codex/session-notes.md`. Run `/session-end` at the end of each work session to append a dated entry and refresh Current Priorities above.
