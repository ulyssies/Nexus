// ============================================================
//  Manual retention sweep: delete untouched discovered jobs older than N days.
//
//  Usage:
...
//
//  Only touches status='new' rows with no applied_at/status update data.
...
if (dryRun) {
  console.log(`[dry run] ${deleted} untouched job(s) older than ${days}d would be deleted.`);
} else {
  console.log(`✓ Deleted ${deleted} untouched job(s) older than ${days}d (status='new' with no application data).`);
}
