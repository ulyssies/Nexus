...
└── server/                  # nested Express package — long-running agent host (port 3001)
    ...
    │   ├── morningBriefAgent.js    # learn interests → NewsAPI → Claude condense → daily brief; buildDigest() synthesizes the home 3-4 sentence digest (cached on morning_brief.digest, re-called only when stale >6h or forced)
...
7. **Home command center ✅ built → ✅ rebuilt as a 5-zone daily dashboard:** `GET /api/home` (`getHome()` in `db/overviewRepo.js`, mounted via `homeRouter` in `routes/overview.js`) aggregates in one read: **Zone 1** alert strip (only time-sensitive/actionable: calendar+email deadlines ≤7d, goals due today, urgent emails — hidden when empty); **Zone 2** today's agenda (active goals with today's check-in status + one-click check-in, a **compact MiniCalendar month-grid widget** — source-colored event chips, click-a-day popover, prev/next month nav, reads the same `/api/calendar` events as the Calendar view — and job deadlines ≤7d); **Zone 3** the morning-brief **digest** — a cached Claude-written digest of **4–6 topic-labeled, actionable sentences** (`**AI & Engineering:**` / `**Career:**` / `**Learning:**`, one concrete line per relevant story, NOT literary synthesis — see the prompt comment in `morningBriefAgent.js`), via `buildDigest()` (regenerated only when stale >6h or forced), with read-more articles, or a "generate brief" button; **Zone 4** the full scrollable agent feed (filter bar all/jobs/email/accountability/brief/archivist + archivist-collapse toggle); **Zone 5** agent health cards (dot from observability + last-run/next-run via cron-parser + one real insight line per agent). `HomeView.jsx` renders all five; `onNavigate` jumps to any tab. The legacy `GET /api/overview` snapshot is kept. Verified live against the real DB (digest synthesis + caching confirmed). ⬜ retake `docs/screenshots/home.png` + `graph.png` (no headless browser in this env).
